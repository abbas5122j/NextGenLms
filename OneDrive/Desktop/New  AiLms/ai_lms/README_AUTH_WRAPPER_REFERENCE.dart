...
class _AuthWrapperState
  ...
  // True only after a successful sign-in when Admin is selected.
  bool _showAdminDashboard = false;

  // Admin-only theme state. The Admin shell owns the visual theme,
  // while this wrapper persists the selected mode across rebuilds.
  ...
  Widget build(BuildContext context) {
    ...
    // ----------------------------------------------------------
    // ADMIN DASHBOARD
    // Role 3 = Admin. Admin is checked before the student dashboard
    // so an Admin can never fall through to the student portal.
    // ----------------------------------------------------------
    ...
    if (_currentScreen ==
      ...
      return LoginForm(
        ...
        onSignInSuccess:
            () {
          setState(() {
            // Admin role is index 3 in the login role selector.
